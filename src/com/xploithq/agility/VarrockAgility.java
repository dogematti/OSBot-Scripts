...
public class VarrockAgility extends Script {
    ...
    private int recover() throws InterruptedException {
        ...
        // 1) Camera wiggle (relative to current angle)
        getCamera().moveYaw(getCamera().getYawAngle() + rand(15, 60));
        getCamera().movePitch(rand(30, 50));
        sleep(rand(200, 400));
        // 2) Step nearby
    ...
    private int handleStamina() throws InterruptedException {
        ...
        if (getSettings().getRunEnergy() < STAMINA_THRESHOLD) {
            // Use contains-match since in-game names are "Stamina potion(4)" etc.
            if (getInventory().interactWithNameThatContains("Drink", "Stamina potion")) {
                sleep(rand(600, 900));
                logf("Sipped stamina.");
            }
        }
    ...
    private void handleObstacle(Obstacle o) throws InterruptedException {
        ...
        if (!obj.isVisible()) {
            ...
            if (!obj.isVisible()) {
                ...
                getCamera().moveYaw(getCamera().getYawAngle() + rand(15, 45));
            }
        }
    ...
    private boolean clickWithActions(RS2Object obj, String... actions) throws InterruptedException {
        ...
        getMouse().click(true); // right-click to open menu
        sleep(rand(120, 220));
        for (String a : actions) {
            if (getMenuAPI().isOpen() && getMenuAPI().selectAction(a)) return true;
        }
        return false;
    }
    ...
    private void maybeAntibanIdle() throws InterruptedException {
        ...
        if (roll < 8) {
            // Wiggle camera relative to current angle
            getCamera().moveYaw(getCamera().getYawAngle() + rand(-40, 40));
            getCamera().movePitch(Math.max(22, Math.min(67, getCamera().getPitchAngle() + rand(-10, 10))));
        } else if (roll < 12) {
    ...
    private void maybeHover(Obstacle next) {
        ...
        if (obj != null && obj.isVisible()) {
            getMouse().move(obj.getPosition().getX(), obj.getPosition().getY());
        }
    }
